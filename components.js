...
export function renderNavbar(user = null) {
    ...
    nav.innerHTML = `
        <div class="container mx-auto px-4 sm:px-6 py-3.5 flex items-center justify-between">
            <a href="Landing.html" class="flex items-center gap-2 group transition-transform active:scale-95">
            ...
            <div class="flex items-center gap-3 sm:gap-6">
                <a href="Landing.html" class="text-slate-600 hover:text-blue-600 font-bold text-[10px] sm:text-xs uppercase tracking-widest flex items-center gap-1.5 transition-colors">
...
export function renderMarksheet(data, meta) {
    ...
    return `
        <div class="a4-container p-4 sm:p-10 border-4 border-double border-slate-800 rounded-lg w-full max-w-4xl mx-auto bg-white transition-all overflow-hidden flex flex-col justify-between h-full">
            <div class="flex-grow">
                <div class="text-center border-b-2 border-slate-800 pb-3 mb-4">
                    ...
                    <p class="text-[9px] sm:text-sm text-slate-600 font-medium mb-2 tracking-wider uppercase">Recognition No: HM-9922 | Established: 1970</p>
                    <div class="bg-slate-900 text-white inline-block px-6 sm:px-10 py-1 sm:py-1.5 rounded-full font-bold uppercase tracking-[0.2em] text-[8px] sm:text-xs">Academic Report Card</div>
                </div>

                <div class="bg-slate-50 p-3 sm:p-5 rounded-xl border border-slate-100 mb-4">
                    <div class="grid grid-cols-2 gap-x-4 sm:gap-x-12 gap-y-2 text-[10px] sm:text-sm">
                        <div class="flex flex-col sm:flex-row sm:items-center gap-1">
                            <b class="text-slate-500 uppercase text-[8px] sm:text-xs">Student Name:</b> 
                            <span class="font-black text-slate-800 truncate">${studentName}</span>
                        </div>
                        <div class="flex flex-col sm:flex-row sm:items-center gap-1">
                            <b class="text-slate-500 uppercase text-[8px] sm:text-xs">Roll Number:</b> 
                            <span class="font-black text-slate-800">${rollNo}</span>
                        </div>
                        <div class="flex flex-col sm:flex-row sm:items-center gap-1">
                            <b class="text-slate-500 uppercase text-[8px] sm:text-xs">Class:</b> 
                            <span class="font-black text-slate-800">${meta.className}</span>
                        </div>
                        <div class="flex flex-col sm:flex-row sm:items-center gap-1">
                            <b class="text-slate-500 uppercase text-[8px] sm:text-xs">Session:</b> 
                ...
                <div class="text-center mb-4">
                    <h2 class="text-[10px] sm:text-lg font-bold text-slate-800 border-b-2 inline-block pb-0.5 border-slate-400 px-6 uppercase tracking-widest italic">${meta.examName}</h2>
                </div>

                <div class="overflow-hidden border border-slate-300 rounded-xl mb-4 shadow-sm">
                    <table class="w-full border-collapse">
                        <thead class="bg-slate-100 text-[9px] sm:text-sm uppercase font-black text-slate-700">
                            <tr>
                                <th class="p-2 sm:p-3 border-r border-slate-300 text-left">Subject Description</th>
                                <th class="p-2 sm:p-3 text-center w-20 sm:w-48">Obtained Marks</th>
                            </tr>
                        </thead>
                        <tbody class="text-[10px] sm:text-sm">
                            ${subjectList.map(([s, m], idx) => `
                                <tr class="${idx % 2 === 0 ? 'bg-white' : 'bg-slate-50/50'}">
                                    <td class="p-1.5 sm:p-2.5 border-r border-b border-slate-200 font-medium text-slate-600 italic truncate">${s}</td>
                                    <td class="p-1.5 sm:p-2.5 border-b border-slate-200 text-center font-black text-slate-900">${m}</td>
                                </tr>
                            `).join('')}
                        </tbody>
                ...
                <div class="grid grid-cols-2 gap-3 sm:gap-6 mb-6">
                    <div class="flex gap-2 sm:gap-4">
                        <div class="flex-1 p-2 sm:p-3 bg-blue-50 border border-blue-100 rounded-2xl text-center shadow-sm">
                            ...
                            <p class="text-sm sm:text-xl font-black text-blue-900">${data['Percentage'] || data['Average'] || 'N/A'}%</p>
                        </div>
                        <div class="flex-1 p-2 sm:p-3 bg-green-50 border border-green-100 rounded-2xl text-center shadow-sm">
                            ...
                            <p class="text-sm sm:text-xl font-black text-green-900">${data['Grade'] || 'N/A'}</p>
                        </div>
                    </div>
                    <div class="${isFailed ? 'bg-red-50 border-red-200 text-red-600' : 'bg-slate-900 border-slate-800 text-white'} p-2 sm:p-3 border rounded-2xl text-center flex flex-col justify-center shadow-lg">
                        ...
                        <p class="text-sm sm:text-xl font-black uppercase tracking-tighter">${(data['Result'] || data['Status'] || 'PASSED').toUpperCase()}</p>
                    </div>
                </div>
            </div>

            <div>
                <div class="grid grid-cols-2 items-end pt-4 border-t-2 border-slate-800 gap-8 sm:gap-40">
                    <div class="text-center group">
                        <img src="https://i.ibb.co.com/HDqfKG6K/Seal-school.png" class="h-16 sm:h-24 mx-auto object-contain transition-transform group-hover:rotate-12">
                    </div>
                    <div class="text-center flex flex-col items-center">
                        <img src="https://i.ibb.co.com/1C0fss2/Tarikul-sign.png" class="h-10 sm:h-16 object-contain mb-1 opacity-90 transition-opacity hover:opacity-100">
                        <div class="border-b-2 border-slate-800 mb-1 w-full max-w-[120px] sm:max-w-[200px] shadow-sm"></div>
                        <p class="font-black text-slate-900 text-[8px] sm:text-[10px] uppercase tracking-tighter whitespace-nowrap">Controller of Examinations</p>
                    </div>
                </div>
                <p class="mt-6 text-center text-[7px] text-slate-400 font-bold uppercase tracking-[0.2em]">* Valid only with official institutional hologram *</p>
            </div>
        </div>
    `;
}
