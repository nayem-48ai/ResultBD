...
export function renderMarksheet(data, meta) {
    ...
    return `
        <div class="a4-container p-10 border-4 border-double border-slate-800 rounded-3xl w-[210mm] min-h-[297mm] bg-white transition-all shadow-2xl flex flex-col justify-between">
            <div class="flex-grow">
                <div class="text-center border-b-2 border-slate-800 pb-3 mb-5">
                    <h1 class="text-4xl font-black text-slate-900 tracking-widest uppercase mb-1">Hat Madhnogor High School</h1>
                    <p class="text-base text-slate-600 font-bold mb-3 tracking-widest uppercase">Recognition No: HM-9922 | Established: 1970</p>
                    <div class="bg-slate-900 text-white inline-block px-12 py-2 rounded-full font-black uppercase tracking-[0.3em] text-sm shadow-lg">Academic Report Card</div>
                </div>

                <div class="bg-slate-50 p-6 rounded-2xl border border-slate-100 mb-6 shadow-inner">
                    <div class="grid grid-cols-2 gap-x-16 gap-y-3 text-base">
                        <div class="flex items-center gap-1.5"><b class="text-slate-400 uppercase text-xs tracking-tighter">Student Name:</b> <span class="font-black text-slate-900 uppercase">${studentName}</span></div>
                        <div class="flex items-center gap-1.5"><b class="text-slate-400 uppercase text-xs tracking-tighter">Roll Number:</b> <span class="font-black text-slate-900">${rollNo}</span></div>
                        <div class="flex items-center gap-1.5"><b class="text-slate-400 uppercase text-xs tracking-tighter">Class:</b> <span class="font-black text-slate-900">${meta.className}</span></div>
                        <div class="flex items-center gap-1.5"><b class="text-slate-400 uppercase text-xs tracking-tighter">Session:</b> <span class="font-black text-slate-900">${meta.year}</span></div>
                    </div>
                </div>

                <div class="text-center mb-5">
                    <h2 class="text-2xl font-black text-slate-900 border-b-2 inline-block pb-1 border-slate-800 px-10 uppercase tracking-[0.2em] italic">${meta.examName}</h2>
                </div>

                <div class="overflow-hidden border-2 border-slate-800 rounded-2xl mb-6 shadow-sm">
                    <table class="w-full border-collapse">
                        <thead class="bg-slate-100 text-sm uppercase font-black text-slate-700">
                            <tr>
                                <th class="p-3 border-r-2 border-b-2 border-slate-800 text-left">Subject Description</th>
                                <th class="p-3 border-b-2 border-slate-800 text-center w-56">Marks</th>
                            </tr>
                        </thead>
                        ...
                        <tfoot class="bg-slate-900 text-white font-black text-sm uppercase tracking-[0.2em]">
                            <tr>
                ...
                <div class="grid grid-cols-2 gap-10 mb-8">
                    <div class="grid grid-cols-2 gap-6">
                        <div class="p-3 bg-blue-50 border-2 border-blue-100 rounded-2xl text-center shadow-sm">
                            <p class="text-blue-600 text-[10px] font-black uppercase tracking-tighter">Percentage</p>
                            <p class="text-3xl font-black text-blue-900">${data['Percentage'] || data['Average'] || 'N/A'}%</p>
                        </div>
                        <div class="p-3 bg-green-50 border-2 border-green-100 rounded-2xl text-center shadow-sm">
                            <p class="text-green-600 text-[10px] font-black uppercase tracking-tighter">Grade</p>
                            <p class="text-3xl font-black text-green-900">${data['Grade'] || 'N/A'}</p>
                        </div>
                    </div>
                    <div class="${isFailed ? 'bg-red-50 border-red-200 text-red-600' : 'bg-slate-900 border-slate-800 text-white'} p-3 border-2 rounded-2xl text-center flex flex-col justify-center shadow-xl transition-all">
                        <p class="${isFailed ? 'text-red-400' : 'text-slate-400'} text-[10px] font-black uppercase tracking-widest">Result Outcome</p>
                        <p class="text-3xl font-black uppercase tracking-tighter">${(data['Result'] || data['Status'] || 'PASSED').toUpperCase()}</p>
                    </div>
                </div>
            </div>
            ...
            <div>
                <div class="grid grid-cols-2 items-end pt-6 border-t-2 border-slate-800 gap-48">
                    <div class="text-center group">
                        <img src="https://i.ibb.co.com/HDqfKG6K/Seal-school.png" class="h-32 mx-auto object-contain transition-transform group-hover:rotate-6">
                    </div>
                    <div class="text-center flex flex-col items-center">
                        <img src="https://i.ibb.co.com/1C0fss2/Tarikul-sign.png" class="h-20 object-contain mb-1 opacity-90 hover:opacity-100 transition-opacity">
                        <div class="border-b-2 border-slate-800 mb-1 w-full max-w-[240px] shadow-sm"></div>
                        <p class="font-black text-slate-900 text-xs uppercase tracking-tighter">Controller of Examinations</p>
                    </div>
                </div>
                <p class="mt-6 text-center text-[10px] text-slate-400 font-bold uppercase tracking-[0.3em]">* VALID ONLY WITH OFFICIAL INSTITUTIONAL HOLOGRAM *</p>
            </div>
        </div>
    `;
}
